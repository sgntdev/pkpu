  ...
  {#each tagihan as data, index (data)}
    <div>
      <div>
        ...
        />
        {#if tagihan.length !== 1}
          <button type="button" on:click={() => handleRemoveTagihan(index)}
            >-</button
          >
        {/if}
        {#if tagihan.length - 1 === index}
          <button type="button" on:click={() => handleAddTagihan()}
      ...
      {#each data.dokumen as dokumen, indexdokumen (dokumen)}
        <div>
          ...
          <button
            type="button"
            on:click={() => handleRemoveDokumen(indexdokumen, index)}>-</button
          >
        </div>
      {/each}
      <button type="button" on:click={handleAddDokumen(index)}
